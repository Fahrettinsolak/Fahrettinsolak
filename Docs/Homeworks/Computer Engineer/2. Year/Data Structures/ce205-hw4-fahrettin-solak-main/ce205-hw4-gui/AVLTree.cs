    ...
    public class AVLTree
    {
        ...
        public Node insert(Node node, string key, int kali, int bagi, int tinggi)
        {
            if (node == null)
            {
                ...
                Circle.Add("circle" + ctrCircle, new Circle(484 / bagi * kali + 1, tinggi, key.ToString()));
                ctrCircle++;
                form.pictureBox1.Invalidate();
        ...
        public Node deleteNode(Node root, string key)
        {
            ...
            if (string.Compare(key, root.key) < 0)
            {
                ...
                delay();
                form.Status.Text = " Current go to left";
                Circle[root.idCircle].brush = new SolidBrush(Color.Black);
                delay();
        ...
        public void find(string value)
        {
            ...
            while (current != null)
            {
                if (string.Compare(value, current.key) < 0)
                {
                    ...
                    Circle[current.idCircle].brush = new SolidBrush(Color.Yellow);
                    delay();
                    form.Status.Text = " Current go to left";
                    Circle[current.idCircle].brush = new SolidBrush(Color.Black);
                    delay();
                    current = current.left;
                }
        ...
        public void inOrderClear(ref Node root, int kali, int bagi, int tinggi)
        {
            ...
            Circle.Add("circle" + ctrCircle, new Circle(484 / bagi * kali + 1, tinggi, root.key.ToString()));
            ...
            if (root.left != null)
            {
                Line.Add("line" + ctrLine, new Line((484 / bagi * kali + 1) + 25, tinggi + 30, (484 / (bagi * 2)) * ((kali * 2) - 1) + 25, tinggi + 100));
                ctrLine++;
                form.pictureBox1.Invalidate();
            }
